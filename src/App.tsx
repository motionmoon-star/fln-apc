...
export default function App() {
  ...
  if (authSession.type === 'candidate' && authSession.candidateId) {
    ...
    if (currentCandidate) {
      return (
        <>
          {notification && (
            <div className="fixed bottom-5 right-5 z-50 bg-slate-900 text-white px-4 py-2.5 rounded-xl shadow-lg border border-slate-700 text-xs font-medium flex items-center gap-2 animate-in fade-in slide-in-from-bottom-4 duration-200">
  ...
  return (
    <div className={`min-h-screen bg-slate-100/70 text-slate-900 flex flex-col font-sans ${language === 'ar' ? 'font-arabic' : ''}`}>
      ...
      {/* Admin Security & Authority Bar - Optimized for Phone & Desktop */}
      <div className="bg-gradient-to-r from-emerald-950 via-slate-900 to-emerald-950 text-white px-3.5 sm:px-6 py-2.5 sm:py-2 border-b border-emerald-900/60 shadow-inner">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center md:justify-between gap-3 md:gap-4 text-xs">
          ...
          <div className="space-y-1.5 min-w-0">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <span className="inline-flex items-center gap-1.5 bg-emerald-900/90 text-emerald-200 px-2.5 py-0.5 rounded-md text-[10px] font-bold border border-emerald-700/80 shadow-2xs shrink-0">
              ...
              <span className="text-emerald-300 font-mono text-[11px] bg-emerald-950/90 px-2 py-0.5 rounded border border-emerald-800/80 font-bold shrink-0">
            ...
            <div className="flex items-center gap-2 min-w-0">
              <div className="w-6 h-6 rounded-full bg-emerald-800 border border-emerald-600/80 text-emerald-100 flex items-center justify-center font-black text-xs shrink-0 shadow-2xs">
              ...
              <div className="min-w-0">
                ...
                <div className="text-[10px] text-emerald-400/90 md:hidden truncate font-medium">
                  {language === 'ar' ? currentAdminUser.roleTitleAr : currentAdminUser.roleTitleFr}
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 md:flex md:items-center gap-2 w-full md:w-auto shrink-0 pt-1 md:pt-0 border-t md:border-t-0 border-emerald-900/60">
            <button
              type="button"
              onClick={() => setIsAdminProfileOpen(true)}
              className="min-h-[44px] md:min-h-0 py-2 md:py-1 px-3 rounded-xl md:rounded-lg text-xs md:text-[11px] font-bold md:font-medium text-emerald-100 bg-emerald-900/90 hover:bg-emerald-800 active:scale-95 border border-emerald-700/80 flex items-center justify-center gap-1.5 shadow-xs transition-all cursor-pointer"
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-3 sm:px-6 lg:px-8 py-3.5 sm:py-5 space-y-3.5 sm:space-y-5 pb-24 md:pb-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2.5 sm:gap-3 bg-white p-3 sm:p-3.5 rounded-xl border border-slate-200 shadow-2xs">
          <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
            <div className="w-2 h-7 sm:w-2.5 sm:h-8 bg-emerald-700 rounded-full shrink-0" />
            <div className="min-w-0">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              ...
              className="inline-flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 text-xs font-semibold rounded-lg bg-slate-50 hover:bg-slate-100 text-slate-700 border border-slate-200 transition-colors cursor-pointer w-full sm:w-auto justify-center"
            >
      ...
      {/* Footer */}
      <footer className="bg-white border-t border-slate-200 py-4 mt-6 sm:mt-8 mb-16 md:mb-0 no-print">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between gap-2 text-xs text-slate-500 text-center sm:text-left">
          <div className="flex items-center gap-2 justify-center">
            <span className="font-bold text-emerald-900">حزب جبهة التحرير الوطني</span>
            <span>•</span>
